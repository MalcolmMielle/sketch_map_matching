...
// 				bool res = cc.planarEditDistanceAlgorithm(gp_copy, gp);
				...
				AASS::VFLGraph graph_vfl_copy = AASS::graphPlace2VFL(gp_copy);
				VF2SubState s0_load(&graph_vfl_copy, &graph_vfl);
				int n_load;
				
// 				std::cout << std::endl << std::endl << "Second test " << std::endl;
				int out = 0;
				std::clock_t a = std::clock();
				if(!match(&s0_load, my_visitor, &out)){
...
// 					cv::waitKey(0);
					...
					vertex_matched_good = vertex_matched_good + out;
