...
BOOST_AUTO_TEST_CASE(trying)
{
	...
	if(index == 0){
		...
		mat_in = cv::imread("../Test/Sequences/mapdrawn.png");
		model = cv::imread("../Test/Sequences/mapdrawn2.png");
		
		cv::threshold(mat_in, mat_in, 50, 255, CV_THRESH_BINARY_INV);
		cv::threshold(model, model, 50, 255, CV_THRESH_BINARY_INV);
	}
	else if(index == 2){
		mat_in = cv::imread("../Test/Sequences/mapdrawn_test.png");
		model = cv::imread("../Test/Sequences/mapdrawn_test-incomplete.png");
		...
		mat_in = cv::imread("../Test/Sequences/mapdrawn_test.png");
		model = cv::imread("../Test/Sequences/mapdrawn_test-incomplete2.png");
		
		cv::threshold(mat_in, mat_in, 50, 255, CV_THRESH_BINARY_INV);
		cv::threshold(model, model, 50, 255, CV_THRESH_BINARY_INV);
	}
	else if(index == 4){
		mat_in = cv::imread("../Test/Sequences/mapdrawn_noloop.png");
		model = cv::imread("../Test/Sequences/mapdrawn_incomplete_noloop.png");
		...
		mat_in = cv::imread("../Test/Sequences/mapdrawn_noloop.png");
		model = cv::imread("../Test/Sequences/mapdrawn_incomplete_noloop2.png");
		...
		model = cv::imread("../Test/Sequences/fullmap.png");
		
		cv::threshold(mat_in, mat_in, 50, 255, CV_THRESH_BINARY_INV);
		cv::threshold(model, model, 50, 255, CV_THRESH_BINARY_INV);
	}
	else if(index == 7){
		mat_in = cv::imread("../Test/Sequences/halfmap.png");
		model = cv::imread("../Test/Sequences/fullmap.png");
		...
		mat_in = cv::imread("../Test/Sequences/secondhalfmap.png");
		model = cv::imread("../Test/Sequences/fullmap.png");
		...
		mat_in = cv::imread("../Test/Sequences/Sketchmapia/SM11_digit.png");
		model = cv::imread("../Test/Sequences/Sketchmapia/SM11_digit_model.png");
		
		cv::threshold(mat_in, mat_in, 50, 255, CV_THRESH_BINARY_INV);
		cv::threshold(model, model, 50, 255, CV_THRESH_BINARY_INV);
	}
	else if(index == 10){
		mat_in = cv::imread("../Test/Sequences/Remastered/0043.jpg");
		model = cv::imread("../Test/Sequences/Remastered/0043_broken.jpg");
		...
		mat_in = cv::imread("../Test/Sequences/Remastered/0043_broken.jpg");
		model = cv::imread("../Test/Sequences/Remastered/0043.jpg");
		...
		mat_in = cv::imread("../Test/Sequences/Remastered/0043.jpg");
		model = cv::imread("../Test/Sequences/Remastered/0043_long.jpg");
...
// 	cv::waitKey(0);
	...
	AASS::SketchMap map(v_real, place_real);
	AASS::SketchMap map_model(v_model, place_model);
	
	map.setObstacleMat(mat_in);
	map_model.setObstacleMat(model);
	
	AASS::probabilisticmatching::ProbabilisticEditDistanceEdge ped;
	AASS::graphmatch::GraphMatcherNeighbor gmn;
	
	//MY THING

	std::cout << "Starting algo4" << std::endl;
	
	ped.algo4(gp_real, gp_model);
	
	std::cout << "Done algo4" << std::endl;
	
	gmn.match(gp_real, gp_model);
	
	std::deque<	
			AASS::graphmatch::Hypothese
		> hypothesis_final = ped.getResult();
	ped.sort(hypothesis_final);
	
	std::deque<	
			AASS::graphmatch::Hypothese
		> hypothesis_final_gmn = gmn.getResult();
	gmn.sort(hypothesis_final_gmn);
	...
	while(aa != -1){
		...
		best_gmn = hypothesis_final_gmn[aa].getDist();
		list_result_gmn = hypothesis_final_gmn[aa].getMatches();
		
		hypothesis_final[aa].drawHypo(gp_real, gp_model, mat_in, model, "ALL FINAL", 2);
		hypothesis_final_gmn[aa].drawHypo(gp_real, gp_model, mat_in, model, "ALL FINAL OLD", 2);
		std::cout << "Distance is : " << best << std::endl;
		std::cout << "Distance old is : " << best_gmn << std::endl;
		cv::waitKey(10);
