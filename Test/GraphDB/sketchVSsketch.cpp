...
int main(int argc,      // Number of strings in array argv
	...
	std::string path("../Test/GraphDB/Sketches/KTH");
	...
	try{
		...
		if(boost::filesystem::is_directory(p)){
			...
			for (std::vector<boost::filesystem::path>::const_iterator it (v.begin()); it != v.end(); it = ++it)
			{
				...
				for (std::vector<boost::filesystem::path>::const_iterator it2 (it); it2 != v.end(); it2 = ++it2)
				{
					boost::filesystem::path fn = *it;
					boost::filesystem::path fn_2 = *it2;
					
					
					std::string name = fn.filename().stem().string();
					std::string name2 = fn_2.filename().stem().string();
					
					if(name != "model"){
						
						cv::Mat test_model = cv::imread(fn.string(), CV_LOAD_IMAGE_GRAYSCALE);
						cv::Mat input = cv::imread(fn_2.string(), CV_LOAD_IMAGE_GRAYSCALE);
