...
AASS::topologicalmap::GraphLine getGraphLineEVG(cv::Mat& testin){
	cv::Mat test;
	testin.copyTo(test);
...
void getGraphLineVoronoi(cv::Mat& testin, AASS::topologicalmap::GraphLine& gl, const std::string& name){
...
// 	cv::imshow("voro model graph", graph_test_voro);
	...
	gl_deux = getGraphLineEVG(test_deux);
...
// 	cv::imshow("evg graph", graph_test_deux);
	...
	getGraphLineVoronoi(test_deux, gl_deux_voro, "model vorn");
...
int main(int argc,      // Number of strings in array argv
	...
	std::string path("../Test/GraphDB/Sketches/Easy");
	cv::Mat test_model = cv::imread("../Test/GraphDB/Sketches/Easy/model.png", CV_LOAD_IMAGE_GRAYSCALE);
...
// 					hypothesis_final_custom[0].drawPartialGraphs(gp_voro, gp_voro_model, input, test_model, na, 1, true);
					...
					std::cout << "Number of links " << hypothesis_final_custom[0].size() << std::endl;
					cv::waitKey(0);
