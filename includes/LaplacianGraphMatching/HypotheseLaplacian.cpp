...
int AASS::graphmatch::HypotheseLaplacian::updateDistance(AASS::graphmatch::GraphLaplacian& gp, AASS::graphmatch::GraphLaplacian& gp_model) const
{
...
// 	std::cout << "updating distance fror hypo of size " << size() << std::endl;
	...
	typedef typename boost::filtered_graph<bettergraph::SimpleGraph<Region, EdgeType>::GraphType, boost::keep_all, boost::function<bool(GraphLaplacian::VertexLaplacian)> > Filtered_laplacian;
	typedef typename boost::graph_traits< Filtered_laplacian>::vertex_iterator VertexIteratorlaplacianFiltered;
	typedef typename boost::graph_traits< Filtered_laplacian>::vertex_descriptor VertexlaplacianFiltered;
	typedef typename boost::graph_traits< Filtered_laplacian>::edge_descriptor EdgelaplacianFiltered;
	typedef typename boost::graph_traits< Filtered_laplacian>::out_edge_iterator EdgeIteratorlaplacianFiltered;



	Filtered_laplacian fp(gp.getGraph(), _keep_all, boost::bind(&StorageFilter::removedVertices, &sf, _1));
	Filtered_laplacian fp_model(gp_model.getGraph(), _keep_all, boost::bind(&StorageFilter::removedVertices, &sf_model, _1));
...
std::tuple<AASS::graphmatch::MatchLaplacian, AASS::graphmatch::MatchLaplacian> AASS::graphmatch::HypotheseLaplacian::getLinkedMatches(const AASS::graphmatch::MatchLaplacian& match_equivalent) {

	//Find equivalent matches
	MatchLaplacian match_here_out;
//	Match match_equi_out;
	MatchLaplacian other_here_out;
	int index_other = -1;
...
//bool AASS::graphmatch::HypotheseLaplacian::shouldReplaceBasedOnCost(const AASS::graphmatch::MatchLaplacian& match_equivalent) {
