#ifndef COMPARATORBASE_MAP
#define COMPARATORBASE_MAP
...
// 			virtual const SketchMap& getMap() const {return _map_one;}
			...
			virtual const std::deque<graphmatch::Hypothese>& getResult() const {return _hypothesis_final;};
...
// 			
			...
			*/
			virtual void sort(std::deque< graphmatch::Hypothese >& hypothesis);
			
			/**
			* @brief Sort the deque from the biggest hypothesis to the smallest one, considering the number of _num_vertex.
			* 
			**/
			virtual void sort_by_size(std::deque< graphmatch::Hypothese >& hypothesis);
			...
			*/
			virtual void pairWiseMatch(const graphmatch::GraphPlace& gp, const graphmatch::GraphPlace& gp2, std::deque< graphmatch::Match >& places_pair);
			...
			* 
			* @param[in] gp_real : graphmatch::GraphPlace input
			* @param[in] gp_model : graphmatch::GraphPlace model
			* @param[in] list_result : deque of Match
			* @param[in] name : image name
			*/
			void drawExportData(graphmatch::GraphPlace& gp_real, graphmatch::GraphPlace& gp_model, const cv::Mat& obstacle, const cv::Mat& obstacle_model, const std::deque< graphmatch::Match>& list_result, const std::string& name);
			...
			*/
			virtual bool match(graphmatch::GraphPlace& gp, graphmatch::GraphPlace& gp2) = 0;
	...
	// 		virtual void draw();
		...
		inline void AASS::graphmatch::GraphMatcherBase::drawExportData(graphmatch::GraphPlace& gp_real, graphmatch::GraphPlace& gp_model, const cv::Mat& obstacle, const cv::Mat& obstacle_model, const std::deque< graphmatch::Match>& list_result, const std::string& name)
		{
		...
		// 		std::cout << std::endl;
				...
				cv::Point2i model;
				model = gp_model[list_result[i].getSecond()].mass_center;
				model.y = model.y + obst_copy.size().height;
				
				cv::Point2i model_normal;
				model_normal = gp_real[list_result[i].getFirst()].mass_center;
		...
		// 	cv::imshow(name + " maps", all_maps);
			mkdir("ExportGraphMatcher", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
			cv::imwrite("ExportGraphMatcher/"+name+".png", all_maps);
